    ...
    <div class="bg-white rounded-xl shadow p-6 border-t-4 border-primary-500">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-xl font-bold text-slate-800">
                <i class="fas fa-edit mr-2 text-primary-500"></i> Editar Servicio
            </h2>
        ...
        <form method="POST" action="<?= url('admin/servicios/actualizar') ?>" class="space-y-4" enctype="multipart/form-data" id="formServicio">
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm text-slate-500">
                        ...
                        Al cambiar el orden, se intercambiará con el servicio que tenga ese número
                    </span>
                </div>
                <p class="text-xs text-slate-400 mt-1">Los números se intercambian automáticamente.</p>
            </div>

            <div class="flex items-center gap-2">
                <input type="checkbox" name="activo" id="activo" <?= $servicio['activo'] ? 'checked' : '' ?>
            ...
            <div class="pt-4 border-t border-slate-200">
                ...
                <div id="subseccionesContainer" class="space-y-3">
                    <?php foreach ($servicio['subsecciones'] ?? [] as $sub): ?>
                        <div class="subseccion-item bg-primary-50/50 rounded-lg p-4 border border-primary-100 relative">
                            ...
                            <div class="grid grid-cols-1 gap-3">
                                ...
                                <div>
                                    ...
                                    <textarea class="subseccion-descripcion w-full border border-slate-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition" 
                                              rows="2"
                                              placeholder="Descripción de la subsección"><?= e($sub['descripcion']) ?></textarea>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ============================================
    // ASIGNAR EVENTOS A SUBSECCIONES EXISTENTES
    // ============================================
    document.querySelectorAll('.subseccion-item .remove-subseccion').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            const item = this.closest('.subseccion-item');
            if (item) {
                item.remove();
            }
        });
    ...
    // Resetear borde al escribir en subsecciones existentes
    document.querySelectorAll('.subseccion-item .subseccion-subtitulo').forEach(function(input) {
        input.addEventListener('input', function() {
            this.style.borderColor = '';
        });
    });
    document.querySelectorAll('.subseccion-item .subseccion-descripcion').forEach(function(textarea) {
